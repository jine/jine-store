  ...
  return (
    <div className="grid relative p-3 bg-white border rounded-md">
      ...
      <div>
        ...
        <h3 className="absolute left-0 top-0 p-3 font-semibold text-foreground text-m">{product.title}</h3>
        {product.brand ? <h3 className="absolute top-8 p-3">{product.brand}</h3> : null}
        <h3 className={`absolute p-3 text-sm uppercase ${product.brand ? 'top-16' : 'top-8'}`}>{product.category}</h3>
